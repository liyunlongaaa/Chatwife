...
def choseChar():
    ...
    for file in file_list:
        ...
        char_list.append(file_name)  #保存人设名字
    print('ID\t人设')
...
def getModel(model_id):
    ...
    for key, info in model_info.items():
        if model_id == 0 and info['language'] == 'Chinese':  #中国声优
            key_list.append(key)
            if 'zh-CN' in key:
                print(colored(str(i) + '\t' + info['name_zh'] + '(' + info['describe'] + ')', 'yellow'))  #Azure的模型
            else:
                print(str(i) + '\t' + info['name_zh'] + '(' + info['describe'] + ')')
            i = i + 1
        elif model_id == 1 and info['language'] == 'Japanese':  #日本声优
            key_list.append(key)
            if 'zh-CN' in key:  # ?
    ...
    voice = int(chose)
    key = key_list[voice]
    return model_info[key]['sid'], key   # sid